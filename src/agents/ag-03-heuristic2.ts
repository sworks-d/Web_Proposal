...
export class Ag03Heuristic2Agent extends BaseAgent {
  ...
  async execute(input: AgentInput): Promise<AgentOutput> {
    ...
    if (part1._parseError || part2._parseError) {
      ...
      return { agentId: this.id, sections: [{ id: 'raw', title: 'ヒューリスティック評価2（生成失敗）', content: rawTexts.slice(0, 10000), sectionType: 'text', isEditable: true, canRegenerate: true }], visualizations: [], metadata: { confidence: 'low', factBasis: [], assumptions: [], missingInfo: [] } }
    }

...
AG-01〜06が既に出力した情報をそのまま再出力しない。
このAGの役割は「前段AGの分析を提案書設計の言語に変換すること」であり
前段AGの内容の転写ではない。

禁止：
✗ AG-04-MERGEのcoreProblemStatementをそのまま再掲する
✗ AG-02-MERGEのsiteDesignPrinciplesをそのまま再掲する（参照IDのみ記載する）
✗ AG-03-MERGEのtopDesignOpportunitiesをそのまま再掲する

出力すべき変換：
✓ analysisMatrix：前段AGの分析を「3層×3フェーズ」の設計根拠として再構成
✓ imageVsReality：前段AGのデータから「訪問者の先入観と実態のギャップ」を抽出
✓ contentArchitecture：AG-06のページ構成に「なぜこのページが必要か」を付与
✓ designPriorities：全分析を統合した「最初に解くべき設計課題」の優先順位
